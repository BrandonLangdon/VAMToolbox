...
class astra3Dabsorption:
    def __init__(self, target: NDArray, params):
        ...
        self.y, self.x = (
            np.mgrid[: target.shape[0], : target.shape[0]] - target.shape[0] // 2
        )
        ...
        if "print_time" in params and "rot_vel" in params:
            ...
            self.q_eff_Navag_planck_freq = (
                params["quantum_eff"] * 1 / (6.022e23 * 6.626e-34 * 7.358e14)
            )
        else:
            self.print_time = 0
            self.rot_vel = 0
        ...
        if self.cuda_available:
            ...
            self.proj_geo = astra.create_proj_geom(
                "parallel3d", 1.0, 1.0, self.nZ, self.pT, self.angles_rad
            )
        else:
            # if not setup a 2D projector for slice-by-slice operation
            self.proj_geo = astra.create_proj_geom(
                "parallel", 1.0, self.pT, self.angles_rad
            )
    ...
    def backProject(self, projections: NDArray) -> NDArray:
        if self.cuda_available:
            ...
            interpolant = interpolate.interp1d(
                np.linspace(0, self.s_per_rot, len(self.angles_rad)),
                self.angles_rad,
                kind="nearest",
            )
            ...
            for angle in angle_iter:
                proj_geom = astra.create_proj_geom(
                    "parallel3d", 1.0, 1.0, self.nZ, self.pT, np.array([angle])
                )

                _, recon_view = astra.creators.create_backprojection3d_gpu(
                    projections[:, [angle_iter.index], :], proj_geom, self.vol_geom
                )
                ...
                Iabs_view = (
                    ...
                    * self.abs_coeff
                    * np.multiply(
                        recon_view / np.max(projections), exp_abs_view[:, :, np.newaxis]
                    )
                )  #
            ...
            reconstruction = (
                reconstruction * self.s_per_rot / len(self.angles_rad)
                + partial_reconstruction
                * self.s_per_rot
                / len(self.angles_rad)
                * partial_reconstruction_time
                / self.s_per_rot
            )
            ...
            for z_i in range(self.nZ):
                ...
                _, reconstruction[:, :, z_i] = astra.creators.create_backprojection(
                    tmp_proj, self.proj_id
                )
    ...
    def getExpabs(self, angle_rad: float) -> float:
        ...
        w = (
            np.real(np.sqrt(self.container_radius**2 - t**2)) - t_perp
        )  # propagation distance inside container
        w[np.absolute(t) > self.container_radius] = (
            np.inf
        )  # set all values outside container radius to infinity
        relative_intensity = np.exp(-w * self.abs_coeff)
        # absorption coefficient in units 1/distance
        # import matplotlib.pyplot as plt
...
if __name__ == "__main__":
    ...
    def createCubeCage(target_bounds, strut_size, side_length, voxel_size):
        ...
        target = np.ones(
            (int(target_bounds / voxel_size), int(target_bounds / voxel_size), 20)
        )
