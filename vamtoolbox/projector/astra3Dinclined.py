...
class astra3Dinclined:
    def __init__(self, target: NDArray, params: dict[str]):
        ...
        for i in range(len(self.angles_rad)):
            ...
            self.angles_vector[i, 9] = np.sin(self.angles_rad[i]) * np.cos(
                inclination_angle
            )
            self.angles_vector[i, 10] = -np.cos(self.angles_rad[i]) * np.cos(
                inclination_angle
            )
    ...
    def backProject(self, projections: NDArray) -> NDArray:
        ...
        projections = np.transpose(projections, (2, 1, 0))
        rec_id, reconstruction = astra.creators.create_backprojection3d_gpu(
            projections, self.proj_geo, self.vol_geom
        )
