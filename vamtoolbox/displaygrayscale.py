...
class EvolvingPlot:
    def __init__(self, target_geo, n_iter, save_img_path=None, vmin=0, vmax=1):
        ...
        self.fig, self.axs = plt.subplots(
            2, 4, gridspec_kw={"height_ratios": [1, 1]}
        )  # Create figure
        ...
        self.target_plot = SlicePlot(
            ...
            ax=self.axs[0, 0],
            fig=self.fig,
            title="Response target, $f_{T}$",
            cmap="gray",
            interpolation="none",
            vmin=vmin,
            vmax=vmax,
        )
        self.recon_plot_dose = SlicePlot(
            np.zeros_like(self.target),
            vol_type="recon",
            ax=self.axs[0, 1],
            fig=self.fig,
            title="Optical dose, $f$",
            cmap="gray",
            vmin=vmin,
            vmax=vmax,
        )
        self.recon_plot_mapped = SlicePlot(
            np.zeros_like(self.target),
            vol_type="recon",
            ax=self.axs[0, 2],
            fig=self.fig,
            title="Dose response, $M(f)$",
            cmap="gray",
            vmin=vmin,
            vmax=vmax,
        )
        self.recon_plot_mapped_error = SlicePlot(
            np.zeros_like(self.target),
            vol_type="recon",
            ax=self.axs[0, 3],
            fig=self.fig,
            title="Response error, $M(f)-f_{T}$",
            vmin=-np.abs(vmax - vmin),
            vmax=np.abs(vmax - vmin),
            cmap="coolwarm",
        )
        self.error_plot = MultiErrorPlot(
            np.full(self.n_iter, np.nan),
            ax=self.axs[1, 0],
            fig=self.fig,
            title="Weighted $L_{p}$ norm loss function",
            xlabel="Iteration",
            ylabel="Cost",
        )
        self.error_lp_plot = MultiErrorPlot(
            np.full(self.n_iter, np.nan),
            np.full(self.n_iter, np.nan),
            np.full(self.n_iter, np.nan),
            np.full(self.n_iter, np.nan),
        ...
        self.error_lp_eps0_plot = MultiErrorPlot(
            np.full(self.n_iter, np.nan),
            np.full(self.n_iter, np.nan),
            np.full(self.n_iter, np.nan),
            np.full(self.n_iter, np.nan),
    ...
    def update(self, loss, dose=None, mapped=None, norms_list=None):
        ...
        if norms_list is not None:
            self.error_lp_plot.update(
                norms_list[0], norms_list[1], norms_list[2], norms_list[3]
            )
            self.error_lp_eps0_plot.update(
                norms_list[4], norms_list[5], norms_list[6], norms_list[7]
            )
...
class EvolvingPlotDemo:
    ...
    def __init__(self, target_geo, n_iter, save_img_path=None):
        ...
        self.fig, self.axs = plt.subplots(
            2, 3, gridspec_kw={"height_ratios": [1, 1]}
        )  # Create figure
        self.fig.set_size_inches(15, 10)  # Resize figure
        manager = plt.get_current_fig_manager()
        manager.full_screen_toggle()
